    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                WHERE user_id=%s AND mission_id=%s AND mission_date=%s
            """, (user_id, mission_id, today))
            row = cur.fetchone()
            if row and row["is_completed"]:
                return False, None

            # 2) 상태 INSERT or UPDATE
            if row:
                cur.execute("""
                ...
                cur.execute("""
                    INSERT INTO UserMissionStatus
                      (user_id, mission_id, mission_date, is_completed)
                    VALUES (%s, %s, %s, TRUE)
                """, (user_id, mission_id, today))
